...
const BinaryState = () => {
  ...
  const saveFlowToServer = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/binaryQ-flow', {
        method: 'POST',
        headers: {
...
const fetchFlowFromServer = async () => {
  try {
    const response = await fetch('http://localhost:3001/get-binaryQ-flow');
    const data = await response.json();
  ...
  const onNodeSelect = useCallback((event, node) => {
    setSelectedNode(node); <button  className="bottom-button">
    EXPORT JSON
</button>
  }, []);
      ...
      <div>
        ...
        <div className='button-out'>
            ...
            <button className="bottom-button">
              <NavLink className="nav-link" to="/">Home Node </NavLink>
              </button>
